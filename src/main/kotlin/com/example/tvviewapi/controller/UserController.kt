...
class UserController(
            ...
            service.findUserByEmail(email)
                  ...
                  .orElseGet {
                        ResponseEntity.notFound()
                              .header("X-Request-ID", "Input data did not match an existing User")
                              .build()
                  }
      ...
      fun updateUser(@RequestBody dto: UserDto): ResponseEntity<UserDto> {
            if (dto.id == null || isNotValidDto(dto)) {
                  return ResponseEntity.badRequest()
                        .header("X-Request-ID", "Input data does not meet requirements")
                        .build()
            }
      ...
      fun createUser(@RequestBody dto: UserDto): ResponseEntity<UserDto> {
            if (dto.id != null || isNotValidDto(dto)) {
                  return ResponseEntity.badRequest().header("X-Request-ID", "Input data does not meet requirements")
                        .build()
            }
